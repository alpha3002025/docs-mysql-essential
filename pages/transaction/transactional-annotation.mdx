...
@Transactional 을 명시한 메서드는 스프링 AOP 프록시를 통해 실행됩니다. 이 프록시는 메서드 호출 전후로 트랜잭션 처리를 연결하고, 내부적으로 TransactionManager 를 이용해 트랜잭션 시작, 커밋, 롤백, 자원 반납 등의 작업을 수행합니다.<br/>

Spring Framework 는 AOP 기반으로 `@Transactional` 이 붙은 메서드/클래스/인터페이스에 대해 커넥션 획득, Commit, Rollback, 자원 반납 등의 작업을 전처리/후처리 하도록 되어 있습니다. 이런 동작으로 인해 간단하게 우리가 작성한 코드의 메서드 위에 `@Transactional` 을 붙이는 것 만으로 해당 메서드 내의 연산은 마치 하나의 연산인 것처럼 원자적 연산을 할 수 있게 됩니다.<br/>
...
스프링의 기본 롤백 정책에서는 체크드, 언체크드 예외인지에 따라서 아래와 같이 커밋/롤백 여부가 달라집니다.
...
- 체크 예외(Checked Exception)
  ...
  - 체크 예외는 `catch{...}` 구문으로 직접 처리하거나 `throws` 로 선언해서 호출한 쪽에서 처리하도록 전달할 수 있습니다.
...
다만 체크 예외를 사용하더라도 `@Transactional` 에 rollbackFor 옵션에 원하는 예외 클래스를 지정하면, 예외가 발생할 때 롤백되도록 설정할 수 있습니다. 반대로 noRollbackFor 옵션을 사용하면 기본적으로 롤백되는 예외에 대해서도 롤백하지 않도록 지정할 수 있습니다.<br/>
...
스프링의 기본 롤백 정책에서는 UncheckedException 발생시에 트랜잭션을 롤백합니다. CheckedException 발생시에는 롤백하지 않고 커밋합니다. 그런데 이런 원칙에 예외를 두는 것 또한 가능합니다. 바로 rollbackFor, noRollbackFor 옵션을 명시해서 특정 익셉션에 대해서 롤백을 하지 않도록 하거나 특정 익셉션에 대해서 롤백을 하도록 지정하는 것이 가능합니다.
...
> `MyHelloPrintException` 은 RuntimeException 클래스를 상속받은 예외 클래스입니다. Runtime Exception 은 UncheckedException 이기에 예외 발생시 기본적으로 롤백을 하겠지만, 아래 코드에서 noRollbackFor 에 `MyHelloPrintException` 을 명시했으므로 `MyHelloPrintException` 이 발생했을 때 트랜잭션을 롤백하지 않게 됩니다.

```java
@Transactional(noRollbackFor = MyHelloPrintException.class)
```
...
예를 들어 JPA 와 같은 라이브러리에서는 읽기전용트랜잭션에서는 엔티티의 변경감지 기능을 수행하지 않기에 스냅샷 객체를 생성하지 않습니다. 읽기와 수정 작업시에 데이터 괴리가 염려되는 로직에서는 읽기 로직에 readOnly 를 사용하는 것도 좋은 방법입니다.<br/>
...
- then : @Transactional 이 적용된 해당 메서드 내에서 예외 발생 이후의 그 다음라인의 로직들이 실행되는지를 테스트 코드에서 Mockito.verify() 를 통해 호출여부를 검증
  - 여기까지는 실행되어야 하고
  - 여기까지는 실행이 되지 말아야 한다 
...
- when : @Transactional 이 적용된 메서드 내에서 Exception 이 발생하는 상황을 Mockito 를 이용해서 가정
- then : @Transactional 이 적용된 해당 메서드의 예외 발생 이후에 대해 테스트 코드에서 Assertion 을 통해서 특정값이 원하는 값이 되는지 검증
